...
prompt.AppendLine("- Login via ConnectAppAndInfo (MUST include `host: window.location.host` in the request body)");
prompt.AppendLine("- ConnectAppAndInfo body: { host: window.location.host, connection: '', login, password, clientAppName, clientAppVersion, clientComputerName }");
